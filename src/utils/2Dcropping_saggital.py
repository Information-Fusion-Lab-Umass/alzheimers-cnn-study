...
dest_dir = "/mnt/nfs/work1/mfiterau/ADNI_data/slice_subsample_no_seg/sagital_noskullstrip/"
...
idx = 0
while not (MCI_count >= 100 and AD_count >= 100 and NC_count >= 100):
    try:
        ...
        if label == "CN" and NC_count < 100:
            ...
            patient_ids.append(x.iloc[idx]['patient_id'])
            patient_labels.append(label)
        elif (label == "AD") and AD_count < 100:
            help_process_data(x.iloc[idx], label)
            AD_count += 1
            patient_ids.append(x.iloc[idx]['patient_id'])
        patient_labels.append(label)

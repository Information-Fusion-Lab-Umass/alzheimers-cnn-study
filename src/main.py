...
def main(config_path, run_id, tb_writer):
    ...
    cross_val_folds = 1
    if "cross_val_folds" in config:
        if config["cross_val_folds"] != "" and config["cross_val_folds"] != False:
            cross_val_folds = int(config["cross_val_folds"])
    cross_val_results = []
    
    for fold_num in range(cross_val_folds):
 
        cross_val_flag = fold_num if cross_val_folds > 1 else -1
        
        path_of_the_weights = None
        if "model_path" in config:
            if config["model_path"] != "" and config["model_path"] != False:
                path_of_the_weights = config["model_path"]
        engine = Engine(config, tb_writer, logger, cross_val_flag, model_path=path_of_the_weights)
        
        if "features_to_pickles" in config:
            if config["features_to_pickles"] != "" and config["features_to_pickles"] != False:
                engine.gen_feature_outputs()
                sys.exit(0)

        logger.log("Cross validation fold (no cross val if it's just 0 of 1): {} of {}"
                        .format(fold_num, cross_val_folds))
        ...
        for epoch in range(num_epochs):
            ...
            train_result, tally = engine.train(epoch=epoch, max_epochs = num_epochs)
        ...
        cross_val_results.append(acc_list)

        # TESTING
        test_percent = None
    
        if config["data"]["test_split"] > 0.0 and config["cross_val_folds"] == "False":
    ...
    if "cross_val_folds" in config:
        if config["cross_val_folds"] != "" and config["cross_val_folds"] != False:
            #logger.log(cross_val_results)
            results = [0] * len(cross_val_results[0])
            for i in range(len(cross_val_results)):
                for j in range(len(cross_val_results[0])):
                    results[j] += cross_val_results[i][j]
            results = [x / len(cross_val_results) for x in results]
            logger.log("Final results, average over the training epochs:")
            logger.log(''.join((str(e) + ", ") for e in results))
...
if __name__ == "__main__":
    ...
    tb_writer = SummaryWriter("{}/{}.log".format(tb_logs_path, args.run_id))
